...
function _point_in_polygon(::Spherical, r_n::StaticArrays.SVector{3, T}, pts) where {T}
    # Project to the tangent plane at r_n and run a 2D PIP. Build any
    # orthonormal basis (e1, e2) ⊥ r_n.
    n = length(pts); n < 3 && return false
    e1 = abs(r_n[3]) < T(0.9) ?
         LinearAlgebra.normalize(StaticArrays.SVector{3, T}(-r_n[2], r_n[1], zero(T))) :
         LinearAlgebra.normalize(StaticArrays.SVector{3, T}(zero(T), -r_n[3], r_n[2]))
    e2 = LinearAlgebra.cross(r_n, e1)
    proj = [StaticArrays.SVector{2, T}(LinearAlgebra.dot(p, e1), LinearAlgebra.dot(p, e2)) for p in pts]
    return _point_in_polygon(GO.Planar(), zero(StaticArrays.SVector{2, T}), proj)
end
